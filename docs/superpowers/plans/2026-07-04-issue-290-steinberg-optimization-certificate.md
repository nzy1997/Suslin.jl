# Issue 290 Steinberg Optimization Certificate Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add an internal Steinberg optimization certificate and exact replay verifier for elementary factor sequence rewrites.

**Architecture:** Add one focused algorithm file, `src/algorithm/redundancy.jl`, for certificate construction, exact product replay, summary generation, rewrite-log count validation, and verification. Include it from `src/Suslin.jl` without exporting any new public API, and extend the existing Steinberg expert test file with catalog-backed no-op and negative-control coverage.

**Tech Stack:** Julia, Oscar matrix and polynomial rings, existing Suslin private elementary factor helpers, existing PR #179 metric helpers, `Test`.

## Global Constraints

- Keep construction internal; do not export the certificate publicly.
- Do not shorten factors yet.
- Use exact multiplication for product checks, not string comparison or factor-count checks alone.
- Reuse `max_elementary_factor_monomial_degree(factors)` and `total_elementary_factor_offdiagonal_monomials(factors)` for before/after metric summaries.
- A no-op certificate on a #288 catalog ordinary-polynomial elementary sequence must verify and record zero `factor_count_delta`.
- Tampering one optimized factor, changing the rule log to a different sequence length, or mixing a factor from a different ring must make verification return false or throw a deliberate `ArgumentError`.

---

## File Structure

- Create `src/algorithm/redundancy.jl`: private `SteinbergOptimizationCertificate`, `_steinberg_optimization_certificate`, exact product replay helpers, rewrite-record normalization, comparison summary generation, and `_verify_steinberg_optimization_certificate`.
- Modify `src/Suslin.jl`: include `algorithm/redundancy.jl` after the existing algorithm includes.
- Modify `test/expert/steinberg_factor_count_optimization.jl`: keep the #289 canonical record tests and add certificate replay/negative-control tests backed by `test/fixtures/steinberg_optimization_cases.jl`.

### Task 1: Certificate Replay And Verification
...
**Interfaces:**
- Consumes: `_canonical_elementary_factor_record(factor)`, `_same_base_ring(left, right)`, `_is_laurent_polynomial_ring(R)`, `max_elementary_factor_monomial_degree(factors)`, and `total_elementary_factor_offdiagonal_monomials(factors)`.
- Produces: non-exported `SteinbergOptimizationCertificate`, `_steinberg_optimization_certificate(original_factors, optimized_factors, applied_rewrite_metadata = ())`, and `_verify_steinberg_optimization_certificate(certificate)::Bool`.

- [ ] **Step 1: Write the failing expert tests**

Append this testset to `test/expert/steinberg_factor_count_optimization.jl` after the existing canonical-record testset:
...
Expected: FAIL with `UndefVarError` for `_steinberg_optimization_certificate` or `SteinbergOptimizationCertificate`, proving the new tests cover missing behavior.

- [ ] **Step 3: Implement the internal certificate**

Create `src/algorithm/redundancy.jl` with this implementation:
...
function _steinberg_summary_core_status(original_factors, optimized_factors, applied_rewrites, original_product, optimized_product)::Bool
...
Add the include to `src/Suslin.jl` immediately after `include("algorithm/quillen_induction.jl")`:
...
Expected: PASS. The test summary includes both Steinberg testsets and the certificate replay testset verifies the no-op catalog certificate and the three negative controls.
...
```bash
git add src/algorithm/redundancy.jl src/Suslin.jl test/expert/steinberg_factor_count_optimization.jl docs/superpowers/plans/2026-07-04-issue-290-steinberg-optimization-certificate.md
git commit -m "feat: add Steinberg optimization certificate"
```

Expected: one implementation commit containing the internal certificate, include, expert tests, and this plan.

## Self-Review

- Spec coverage: Task 1 covers the internal certificate, exact replay verifier, PR #179 metrics, no-op #288 catalog certificate, rule-log count checking, product tampering, mixed-ring rejection, and no public export.
- Placeholder scan: no `TBD`, `TODO`, or deferred implementation instructions remain.
- Type consistency: the plan consistently uses `SteinbergOptimizationCertificate`, `_steinberg_optimization_certificate`, and `_verify_steinberg_optimization_certificate`.
