# Issue 334 Case008 d14 Laurent Link-Witness Search Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add an expert-only bounded Laurent link-witness search report for the replay-derived post-descent `case_008 d=14` column.

**Architecture:** Keep all new search/report logic in one expert test file, matching #329 and #333. The report consumes the #333 context and recomputes the #329 post-descent column by replay, scans the declared radius-1 pivot-plus-shifted-partner family, and validates accepted candidates by exact replay.

**Tech Stack:** Julia, Test, Oscar, Suslin.jl test-only helpers.

## Global Constraints

- Do not add a production Laurent ECP stage or public API.
- Do not widen beyond `exponent_radius == 1`.
- Candidate family is exactly `:two_entry_laurent_combination`.
- Candidate semantics are exactly `:pivot_plus_shifted_partner_endpoint`.
- The replay convention is `pivot_entry + coefficient * u^a * v^b * partner_entry` in declared `ring_generators` order.
- Do not silently count reverse pivot/partner direction under the same family.
- The checked d14 count is exactly `13 * 9 * 1 == 117`.
- If exhausted, report `next_boundary == :laurent_link_witness_search_expansion` and do not claim endpoint-reduction progress.

---

### Task 1: Add the Failing Search Contract

**Files:**
- Create: `test/expert/case008_d14_laurent_link_witness_search.jl`
- Modify: `test/runtests.jl`

**Interfaces:**
- Consumes: `case008_d14_laurent_link_witness_context(report; source_validation)`
- Consumes: `validate_case008_d14_laurent_link_witness_context(context, report; source_validation)::Symbol`
- Produces later: `case008_d14_laurent_link_witness_search_report()`
- Produces later: `validate_case008_d14_laurent_link_witness_search_report(report)::Symbol`
- Produces later: `verify_laurent_link_witness_candidate(column, R, candidate)::Bool`

- [ ] **Step 1: Create the failing test file**

Create `test/expert/case008_d14_laurent_link_witness_search.jl` with this initial content:
...
Expected: nonzero exit with `UndefVarError: case008_d14_laurent_link_witness_search_report not defined`.

---

### Task 2: Implement Bounded Search and Validation
...
- Produces: `case008_d14_laurent_link_witness_search_report(fixture = ToricBuilderCase008D14ColumnBoundary.boundary_fixture())::NamedTuple`
- Produces: `validate_case008_d14_laurent_link_witness_search_report(report, fixture = ToricBuilderCase008D14ColumnBoundary.boundary_fixture())::Symbol`
- Produces: `verify_laurent_link_witness_candidate(column, R, candidate)::Bool`

- [ ] **Step 1: Add constants and field helpers above the testset**

Add these definitions after the dependency include:
...
- [ ] **Step 2: Add witness and endpoint replay helpers**
...
- [ ] **Step 3: Add candidate verifier**

Add this verifier below the replay helpers:
...
- [ ] **Step 4: Add search report and validator**

Add these definitions below the verifier:
...
### Task 3: Add Negative Controls and Final Verification
...
- Consumes: `validate_case008_d14_laurent_link_witness_search_report(report)::Symbol`

- [ ] **Step 1: Add small mutation helpers above the testsets**
...
- [ ] **Step 2: Strengthen the d14 report testset**

Inside the existing d14 testset after the conditional status block, add:
...
- [ ] **Step 3: Add synthetic verifier controls**

Append this second testset:
...
Expected: both Julia commands exit 0 and `git diff --check` prints no output.
...
```bash
git add docs/superpowers/plans/2026-07-08-issue-334-case008-d14-laurent-link-witness-search.md test/expert/case008_d14_laurent_link_witness_search.jl test/runtests.jl
git commit -m "Add case008 d14 Laurent link-witness search"
```
