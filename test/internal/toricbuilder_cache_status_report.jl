...
function _case008_details_prove_algorithm_boundary_progress(details::AbstractString)
    ...
    return any(<(CASE008_D15_BOUNDARY_DIMENSION), column_lengths)
end

function _case008_details_prove_timeout_progress(details::AbstractString)
    _case008_details_prove_algorithm_boundary_progress(details) && return true
...
function _case008_bounded_route_row_passed_old_d15_boundary(row; timeout_seconds)
    ...
    row.route_status == :certified_algorithm_boundary &&
        return _case008_details_prove_algorithm_boundary_progress(details)
    if row.route_status == :timed_out
        stages = _matching_stage_names(row, :timed_out)
        length(stages) == 1 || return false
        stage = only(stages)
        stage in (:certificate_construction, :verification) || return false
        return _case008_details_prove_timeout_progress(details)
    end
    return false
end
...
@testset "ToricBuilder cache Q-block status report" begin
    ...
    post_d15_stage_timings = merge(ToricBuilderCacheQBlockStatusReport._not_run_stage_timings(), (;
    ...
    boundary_stage_timings = merge(post_d15_stage_timings, (; certificate_construction = boundary_stage_timings.certificate_construction))
    ...
    current_d15_boundary_row = merge(old_d15_boundary_row, (;
        ...
        stage_timings = merge(post_d15_stage_timings, (;
            certificate_construction = (;
                status = :certified_algorithm_boundary,
                elapsed_seconds = 0.125,
                error_details = current_d15_boundary_details,
    ...
    completed_d15_boundary_details =
        "peel progress: completed steps=16, last completed d=15; unsupported_laurent_column_family"
    completed_d15_boundary_row = merge(old_d15_boundary_row, (;
        error_details = completed_d15_boundary_details,
        stage_timings = merge(post_d15_stage_timings, (;
            certificate_construction = (;
                status = :certified_algorithm_boundary,
                elapsed_seconds = 0.125,
    ...
    @test _bounded_route_row_is_structured(completed_d15_boundary_row; timeout_seconds = 1.0)
    @test !_case008_bounded_route_row_passed_old_d15_boundary(
        completed_d15_boundary_row;
        timeout_seconds = 1.0,
    )
    ...
    verification_timeout_without_progress_row = merge(case008_timeout_row, (;
        route_status = :timed_out,
        error_details = "timed out after 1.000 seconds while running verification",
        stage_timings = merge(post_d15_stage_timings, (;
            certificate_construction = (;
                status = :pass,
                elapsed_seconds = 0.125,
    ...
    @test _bounded_route_row_is_structured(
        verification_timeout_without_progress_row;
        timeout_seconds = 1.0,
    )
    @test !_case008_bounded_route_row_passed_old_d15_boundary(
        verification_timeout_without_progress_row;
